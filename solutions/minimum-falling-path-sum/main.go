...
func minFallingPathSum(matrix [][]int) int {
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			memo[i] = append(memo[i], min)
		}
	}

	for i := 0; i < n; i++ {
		minSum := calculateMinFallingPathSum(matrix, 0, i, memo)
		if minSum < min {
			min = minSum
		}
	}
...
func calculateMinFallingPathSum(matrix [][]int, startRow, startCol int, memo [][]int) int {
	n := len(matrix)
	...
	if val := memo[startRow][startCol]; val != math.MaxInt {
		return val
	}

	left := calculateMinFallingPathSum(matrix, startRow+1, startCol-1, memo)
	below := calculateMinFallingPathSum(matrix, startRow+1, startCol, memo)
	right := calculateMinFallingPathSum(matrix, startRow+1, startCol+1, memo)

	val := matrix[startRow][startCol]

	if left == math.MaxInt && below == math.MaxInt && right == math.MaxInt {
		return val
	}
